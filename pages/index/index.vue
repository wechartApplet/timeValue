  ...
  <view class="container">
    ...
    <view class="result" v-if="showResults">
      ...
      <view class="result-item">
        ...
        <text class="result-value">{{ gaokaoDate }}，年龄为 {{ ageAtGaokao }} 岁</text>
      </view>

      <view class="result-item">
        <text class="result-label">你本科毕业的时间为：</text>
        <text class="result-value">{{ bachelorDate }}，年龄为 {{ ageAtBachelor }} 岁</text>
      </view>

      <view class="result-item">
        <text class="result-label">你硕士毕业的时间为：</text>
        <text class="result-value">{{ masterDate }}，年龄为 {{ ageAtMaster }} 岁</text>
      </view>
      ...
      <view class="result-item">
        <text class="result-label">你赚够一辆五十万小车所需要的时间为：</text>
        <text class="result-value">{{ timeForCar }} 年，那时你的年龄为 {{ ageAtCar }} 岁</text>
      </view>

      <view class="result-item">
        <text class="result-label">你准备好30W彩礼交且花十万办婚礼娶到一个玩够了且不爱你的大陆汉女所需要的时间为：</text>
        <text class="result-value">{{ timeForMarriage }} 年，那时你的年龄为 {{ ageAtMarriage }} 岁</text>
      </view>
      ...
      <view class="result-item">
        ...
        <text class="result-value">{{ countdown.years }} 年 {{ countdown.months }} 月 {{ countdown.days }} 日 {{ countdown.hours }} 时 {{ countdown.minutes }} 分 {{ countdown.seconds }} 秒</text>
      </view>
    </view>
  </view>
</template>
...
import { calculateSalary } from "./calculateSalary"; // 导入计算每秒收入和赚一分钱所需时间的模块
import { calculateTimeToAchieve } from "./calculateTimeToAchieve"; // 导入计算赚够特定金额所需时间的模块
import { calculateAge } from "./calculateAge"; // 导入计算当前年龄的模块
import { calculateGaokaoDate, calculateBachelorDate, calculateMasterDate } from "./calculateEducationDates"; // 导入计算高考、本科、硕士毕业日期的模块
import { calculateSavings } from "./calculateSavings"; // 导入计算当前储蓄和退休前储蓄的模块
import { startCountdown } from "./calculateCountdown"; // 导入计算倒计时的模块
...
export default {
  setup() {
    ...
    const calculateTimeValue = () => {
      ...
      // 启动倒计时
      const countdownInterval = startCountdown(currentAge.value);
      onUnmounted(() => clearInterval(countdownInterval)); // 组件销毁时清除倒计时
    };
